...
const articles = {
  ...
  create: (req, res) => {
    ...
    if (validationResult.isInvalid > 0) {
      ...
      firebaseDB.ref(`/articles/${articleID}/`).set(data, error => {
        if (error) {
          res.json(common.response(500, `Data could not be saved. | ${error}`));
        } else {
          res.json(common.response(201, 'Data saved successfully.', { id: articleID }));
        }
      });
    }
  }
